...
class MovePlatform:
    def __init__(self):
        ...
        self._set_linear_effort(-0.25, self.cmd_z, 2, self.goal_tolerance_z, 70, 10)
        time.sleep(3)
    ...
    def _set_linear_effort(self, goal_p, pub_msg, j, goal_tolerance, balance_effort =0, effort_factor=1.0):
        ...
        while abs(goal_p - self.joint_p[j])>goal_tolerance:
            if j==0 and self.joint_p[1]*self.joint_p[1]+self.joint_p[0]*self.joint_p[0]>0.3+self.goal_tolerance_xy:
                print('out of circle')
                self.r.sleep()
                break
    ...
    def up_down(self):
        self._set_linear_effort(0.0, self.cmd_z, 2, self.goal_tolerance_z, 70, 10)
        # self._set_linear_effort(-0.5, self.cmd_z, 2, self.goal_tolerance_z, 70, 10)
        self._set_linear_effort(-0.25, self.cmd_z, 2, self.goal_tolerance_z, 70, 10)
        print('finished up down')

    def left_right(self):
        self._set_linear_effort(0.45, self.cmd_x, 0, self.goal_tolerance_xy)
        self._set_linear_effort(-0.55, self.cmd_x, 0, self.goal_tolerance_xy)
    ...
    def forward_backward(self):
        self._set_linear_effort(0.47, self.cmd_y, 1, self.goal_tolerance_xy)
    ...
    def random_z(self):
        for i in range(self.xyz_itr):
            ...
            self._set_linear_effort(posi_z, self.cmd_z, 2, self.goal_tolerance_z, 70, 10)
        print("random xyz finished")
        self.xyz_finished=True
        return
    ...
    def random_yaw(self):
        for i in range(self.rpy_itr):
            if self.rpy_finished or self.xyz_finished:
                return
            y = random.randint(-150, 150)/100.0
            print('random yaw = ',y)
